...
| 6 | Backups & DR | ⚠ (mirror exists, transfer + PITR pending) | RUNBOOK covers deploy / rollback / incident response / PITR restoration / RLS role swap / common issues. **Live source is at github.com/VIAR-Devs/IW-Generator** (Riaz's secondary GH org) with gardenprojectx as collaborator. Charter Item 6 technically satisfied (off-site mirror exists), but the repo and the Replit project both sit under Riaz's accounts, not Garden's. PITR confirmation depends on Neon plan tier on the live project. | Action: Riaz transfers the GH org + the Replit project to Garden's accounts; confirm Neon PITR enabled. |
...
| 8 | Bus factor & continuity | ❌ (audit pending) | Known today: Irfan + Riaz have GH access via gardenprojectx + their own accounts. Need to confirm Humza. The VIAR-Devs GH org + the Replit project sit on Riaz's secondary accounts; transfer to Garden ownership is pending Riaz's actioning. Neon / Stripe / Resend / Anthropic admin coverage TBC. | Action: Irfan confirms admin coverage across 6 systems. Minimum bar = at least 2 of {Irfan, Riaz, Humza} on every production system. Document in README ownership section. |
...
Progress vs 13 May (2 green / 5 amber / 1 red): Items 4 and 5 promoted from amber to green-on-merge. Item 6 is the only structural gap left, blocked on Riaz actioning the GH org + Replit project transfer to Garden ownership.
...
3. **Item 6 repo + Replit transfer** — Riaz (who owns the VIAR-Devs GH org and the Replit project) actions the transfer to gardenprojectx (GH) and to Garden's Replit account. In flight as of 14 May — Riaz hit a snag on the Replit side and is picking it up first thing this morning.
